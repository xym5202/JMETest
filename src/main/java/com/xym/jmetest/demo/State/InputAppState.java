...
public class InputAppState extends BaseAppState implements ActionListener {
    ...
    private void walk() {
        ...
        Vector3f frontDir=tempVars.vect1.set(0,0,0.6f);
        Vector3f leftDir=tempVars.vect2.set(0.4f,0,0);
        Vector3f walkDir=tempVars.vect3.set(0,0,0);

        if (forward){
            walkDir.addLocal(frontDir);
        }
        if (backward){
            walkDir.addLocal(frontDir.negate());
        }
        if (left){
